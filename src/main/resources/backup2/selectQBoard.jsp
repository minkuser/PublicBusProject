								...
								<table class="visual-tbody">
									<tr class="point">
										...
										<td class="type3 th"><select style="font-size: 30px;"
											id="category" name="category" <c:if test="${sessionId ne QBoard.users || maxLayer gt 0}">disabled="disabled"</c:if>>
												<option value="USER"
									...
									<tr class="point">
										...
										<td class="title4 th"><input placeholder="제목을 입력해 주세요"
											style="font-size: 30px;" type="text" name="title"
											value="${QBoard.title}" size="52" <c:if test="${sessionId ne QBoard.users || maxLayer gt 0}"> readonly="readonly"</c:if>/></td>
									</tr>
									<tr class="point">
										<td class="cont2 th">내용</td>
										<td class="cont3 th"><textarea placeholder="내용을 입력해 주세요"
												style="font-size: 30px;" name="content" rows="5" cols="40" <c:if test="${sessionId ne QBoard.users || maxLayer gt 0}"> readonly="readonly"</c:if>>${QBoard.content}</textarea></td>
									</tr>
										...
										<tr>
											...
											<td class="file1 th"><a
												...
												<button
													...
													onclick="fileDelete()" <c:if test="${sessionId ne QBoard.users || maxLayer gt 0}">disabled="disabled"</c:if>>
													<span
									...
									<tr>
										<td class="down th">첨부파일</td>
										<td class="down1 th"><input type="file" name="uploadFile" <c:if test="${sessionId ne QBoard.users || maxLayer gt 0}">disabled="disabled"</c:if>></td>
									</tr>
									<tr>
								...
								<div class="button-container"
									...
									<c:if test="${sessionId ne 'ad'}">
										 <c:if test="${sessionId eq QBoard.users}">
											<button
											style="width: 100px; margin: auto; display: inline-block; border: 1px solid; padding: 15px 30px; border-radius: 15px; margin-top: 20px;"
											onclick="location.href='insertQBoard.do'">새글</button>
										...
										<c:if test="${sessionId ne QBoard.users}">
											<button
											style="width: 100px; margin: auto; display: inline-block; border: 1px solid; padding: 15px 30px; border-radius: 15px; margin-top: 20px;"
											onclick="location.href='qboard/insertQBoard.do'">새글</button>
										</c:if>
									</c:if>	
									<button
										style="width: 100px; margin: auto; display: inline-block; border: 1px solid; padding: 15px 30px; border-radius: 15px; margin-top: 20px;"
